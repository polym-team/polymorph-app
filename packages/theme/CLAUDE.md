...
앱을 하나씩 이 시스템으로 옮길 때, **색의 성격**으로 분류해 처리한다 (jibsayo가 레퍼런스):

| 성격 | 예 | 처리 |
|---|---|---|
| 브랜드/UI | nav 활성, 링크, 선택 마커, 소프트 hover | → `primary` (`text-primary`, `bg-primary/5` 등) |
| 시맨틱(에러) | 에러 박스, 삭제/로그아웃 | → `danger` (`bg-danger/10 text-danger`) |
| 도메인 고유색 | jibsayo 가격 상승/하락(빨강/파랑) | → **앱 로컬 토큰** (presets/<app>.css + 앱 tailwind 확장). shared theme에 넣지 말 것 |
| 장식/아이콘 | 즐겨찾기 별(노랑) | **raw 유지** |
| 데이터-비주얼 | 차트 hex 팔레트 | **raw 유지** |
| 뉴트럴 | gray 계열 | 대부분 유지 (필요 시 `muted`/`border`) |

핵심: **브랜드·시맨틱만 shared 토큰으로. 도메인 고유색은 앱 로컬 토큰. 장식·차트는 손대지 않는다.**
잘못 옮기면 의미가 깨진다 (예: 가격 파랑을 primary로 바꾸면 상승/하락 구분이 사라짐).
