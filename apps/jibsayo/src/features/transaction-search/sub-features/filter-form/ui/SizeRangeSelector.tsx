...
export function SizeRangeSelector({
  ...
  const SLIDER_MAX = SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE + 1;

  // 외부 값 변경 시 동기화
  useEffect(() => {
    setLocalMin(minSize ?? SEARCH_PARAM_CONFIGS.SEARCH_MIN_SIZE);
    setLocalMax(maxSize ?? SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE);
  }, [minSize, maxSize]);
  ...
  const positionToValue = useCallback(
    (x: number): number | typeof Infinity => {
      if (!sliderRef.current) return SEARCH_PARAM_CONFIGS.SEARCH_MIN_SIZE;
      ...
      const value = Math.round(
        SEARCH_PARAM_CONFIGS.SEARCH_MIN_SIZE +
          ratio * (SLIDER_MAX - SEARCH_PARAM_CONFIGS.SEARCH_MIN_SIZE)
      );

      // SLIDER_MAX(51)는 Infinity로 변환
      return value === SLIDER_MAX
        ? Infinity
        : Math.min(value, SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE);
    },
  ...
  const valueToPosition = (value: number): number => {
    ...
    const ratio =
      (effectiveValue - SEARCH_PARAM_CONFIGS.SEARCH_MIN_SIZE) /
      (SLIDER_MAX - SEARCH_PARAM_CONFIGS.SEARCH_MIN_SIZE);
    // 핸들 반지름을 고려해서 위치 계산
    const effectiveWidth = sliderWidth - HANDLE_RADIUS * 2;
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      if (dragging === 'min') {
        ...
        const effectiveMax =
          localMax === Infinity
            ? SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE
            : Math.min(localMax, SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE);
      ...
      const newMin = Math.max(
        ...
        Math.min(
          clickValue === Infinity
            ? SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE
            : clickValue,
          Math.min(effectiveLocalMax, SEARCH_PARAM_CONFIGS.SEARCH_MAX_SIZE)
        )
      );
      setLocalMin(newMin);
