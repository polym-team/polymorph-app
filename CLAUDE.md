...
| `/Users/rootbeer.axz-pc/Documents/project/polymorph-upptime` | [Upptime](https://upptime.js.org) 기반 상태 페이지. GitHub Actions가 5분마다 `*.polymorph.co.kr` 엔드포인트를 체크해서 `history/`에 git commit하고, gh-pages 브랜치로 svelte 상태 페이지를 자동 배포. 클러스터/Cloudflare 다운 시에도 GitHub Pages가 살아있어 외부 가시성 확보. `.upptimerc.yml`만 편집하면 됨. |
| `/Users/rootbeer.axz-pc/Documents/project/polymorph-maintenance` | Cloudflare Worker failover 대상 정적 페이지(`index.html` 한 장). origin이 5xx일 때 Worker가 이 페이지를 fetch해서 사용자에게 응답. client-side JS가 `window.location.host`로 서비스명 자동 매칭. |
...
- 모니터링 대상 사이트 추가/수정 → **polymorph-upptime/.upptimerc.yml**
- 장애 시 사용자에게 보일 메인터넌스 페이지 수정 → **polymorph-maintenance/index.html**
