...
- **✅ G. UI (완료)**: FIDS(Solari 스플릿-플랩) 전광판 룩. Artifact 목업 승인 후 이식. `SplitFlap` 자체 구현(랜덤풀 사이클링, 한글 지원 — 순차 alphabet 라이브러리 부적합). 예정=애니메이션/지난=정적, 모노 서비스+상태색 강조, 모바일 카드. 빌드/tsc/lint 통과.
- **✅ B. k8s 배포 (완료·푸시됨 2026-07-13)**: Dockerfile+워크플로우(polymorph-app), argocd app+manifests+secret(polymorph-k8s), oauth-server-secret에 2키 추가. 두 repo 푸시(k8s 먼저)→ 워크플로우 성공, argocd 롤아웃(image c23adb6). 남은 수동: **DNS `myflighthistory.polymorph.co.kr`(Cloudflare)**, argocd Healthy 확인, prod E2E.
